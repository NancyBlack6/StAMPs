...
"""调用函数模型"""
clf = joblib.load(r"./ML/trained model/GaussianNB.m")
...
print(y_pred)
print(pred_prob)
pd.DataFrame(y_pred).to_csv(r"./result/predict result\each model/NB.csv", index=False)
pd.DataFrame(pred_prob).to_csv(r"./result/predict result\each model/prob/NB.csv", index=False)