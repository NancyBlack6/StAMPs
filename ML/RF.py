...
"""调用函数模型"""
clf = joblib.load(r"./ML/trained model/RF.m")
...
print(y_pred)
print(pred_prob)
pd.DataFrame(y_pred).to_csv(r"./result/predict result\each model/RF.csv", index=False)
pd.DataFrame(pred_prob).to_csv(r"./result/predict result\each model/prob/RF.csv", index=False)