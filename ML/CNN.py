...
"""调用函数模型"""
model = load_model(r'./ML/trained model/CNN_best_classification_model.h5')
print(Feature)
print("------------------------------------------------------------------")
print(X)
...
print("------------------------------------------------------------------")
pd.DataFrame(y_pred).to_csv(r"./result/predict result\each model/CNN.csv", index=False)
pd.DataFrame(pred_prob).to_csv(r"./result/predict result\each model/prob/CNN.csv", index=False)
