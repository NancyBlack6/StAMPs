...
print(Feature)
"""调用函数模型"""
clf = joblib.load(r"./ML/trained model/LGBM.m")
...
print(y_pred)
print(pred_prob)
pd.DataFrame(y_pred).to_csv(r"./result/predict result\each model/LGBM.csv", index=False)
pd.DataFrame(pred_prob).to_csv(r"./result/predict result\each model/prob/LGBM.csv", index=False)